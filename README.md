...
- `main` push와 `v*` 태그 push는 **릴리스를 발행**하고 APK를 첨부합니다. 버전은 `1.0.0.<run_number>` 형태입니다.
- 앱은 시작할 때(설정에서 끌 수 있음) 그리고 **설정 → 앱 업데이트**에서 GitHub 릴리스를 확인하고, 더 높은 버전이면 내려받아 설치합니다.

### 서명 키 (권장, 업데이트 설치에 필요)

키가 없으면 CI는 debug 키로 서명합니다. debug 키는 빌드마다 달라져 **덮어쓰기 설치가 실패**하고 Google 로그인도 막힙니다. 한 번만 만들어 두세요.
